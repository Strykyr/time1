...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        #water
        # dict = ['120','200','400','120','220','340','120','220','340']
        ...
        for j in range(len(self.test_data)):
            ...
            # door
            # f.write(dict[j] + "new >>> temperature>>>>>>>>>>>>>>>>>>>>>>." + "  \n")
            # f.write(dict[j] + "new >>>> ceiling temperature>>>>>>>>>>>>>>>>>>>>>>." + "  \n")
            # water+exhaust
            f.write(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+"\n")
            f.write(str(dict[k]) + "senior222_temperature>>>>>>>>>>>>>>>>>>>>>>." + "  \n")
